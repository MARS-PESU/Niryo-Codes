...
# - Set up
workspace1 = "ws_multirobot_line1"  # Name of your workspace
conf_file_name = "saved_joints_poses.yaml"

# robot 1
front_ned_ip_address = "192.168.132.33"  # "192.168.1.26"  # Replace by front Ned ip address

# robot 2
back_ned_ip_address = "192.168.132.236"  # Replace by back Ned ip address
...
class RobotsMains:
    def __init__(self, front_ned, back_ned, workspace, saved_joints_poses):
        ...
        self.client1.vision.set_brightness(vision_brightness)
    ...
    def run(self):
        self.conveyor_id = self.client1.conveyor.set_conveyor()
...
class RobotLoop(Thread):
    ...
    def run(self):
        self.client.arm.move_joints(sleep_joints)
        self.robot_loop()
...
class FrontNed(RobotLoop):
    ...
    def robot_loop(self):
        ...
        self.client.tool.update_tool()
        self.client.tool.release_with_tool()
        self.client.conveyor.control_conveyor(self.parent.conveyor_id, False, conveyor_speed, ConveyorDirection.BACKWARD)

        while True:
            # run the conveyor until a pawns get in the workspace
            while True:
                obj_found, *_ = self.client.vision.vision_pick(workspace1, z_offset, shape=ObjectShape.ANY,
            ...
            self.client.arm.move_joints(self.saved_joints_poses["client1_intermediate_pos"])  # up
            print("Front Ned | dropping pawn ")
            self.client.arm.move_joints(self.saved_joints_poses["drop_positions_of_client1"])  # drop
            self.client.tool.release_with_tool()
            self.parent.stock += 1

            self.client.arm.move_joints(self.saved_joints_poses["client1_observation_pose"])
    ...
    def wait_obj(self):
        self.client.arm.move_joints(self.saved_joints_poses["client1_observation_pose"])  # observation
        obj_found, pos, shape, color = self.client.vision.detect_object(workspace1, shape=ObjectShape.ANY,
                                                                 color=ObjectColor.ANY)
        if obj_found and pos.x < 0.90:
            return
        self.client.conveyor.control_conveyor(self.parent.conveyor_id, True, conveyor_speed, ConveyorDirection.BACKWARD)
        while obj_found is False or pos.x > 0.90:
            t = time.time()
            obj_found, pos, shape, color = self.client.vision.detect_object(workspace1, shape=ObjectShape.ANY,
                                                                     color=ObjectColor.ANY)
            self.parent.conveyor_move_time += time.time() - t
        time.sleep(0.3)
        self.client.conveyor.control_conveyor(conveyor_id, False, 0, ConveyorDirection.BACKWARD)
...
class BackNed(RobotLoop):
    ...
    def robot_loop(self):
        ...
        while True:
            ...
            self.client.arm.move_joints(self.saved_joints_poses["client2_intermediate_pos"])  # up
            print("Back Ned | grab a pawn ")
            self.client.arm.move_joints(self.saved_joints_poses["pick_positions_of_client2"])  # grab
            self.client.tool.grasp_with_tool()
            self.parent.stock -= 1
            ...
            self.client.arm.move_joints(tmp)  # drop
...
def ask_position():
    ...
    client1.arm.move_joints(*sleep_joints)
    client2.arm.move_joints(*sleep_joints)
    client1.arm.set_learning_mode(True)
    client2.arm.set_learning_mode(True)
    ...
    function = [[nothing],
                [nothing],
                [client1.tool.release_with_tool],
                [client2.tool.grasp_with_tool],
                [nothing],
                [client2.tool.release_with_tool]]
    ...
    for question_index in range(len(questions)):
        ...
        client[question_index].arm.set_learning_mode(True)
...
def main():
    ...
    ws_list = client1.vision.get_workspace_list()
...
if __name__ == '__main__':
    ...
    calib_thread_r1 = Thread(target=client1.arm.calibrate, args=[CalibrateMode.AUTO, ])
    calib_thread_r2 = Thread(target=client2.arm.calibrate, args=[CalibrateMode.AUTO, ])
    calib_thread_r1.start()
    calib_thread_r2.start()
    calib_thread_r1.join()
    calib_thread_r2.join()

    client1.tool.update_tool()
    client2.tool.update_tool()
