...
# ----------------------------------------Part to change ------------------------------------------------
leader_ip = "192.168.132.33"  # ip of the Leading robot
clients_ip = ["192.168.132.236"]  # ips of the followers
# -------------------------------------------------------------------------------------------------------
...
class RobotsThreading:
    ...
    def run(self):
        ...
        for client_thread in robot_clients_threads:
            client_thread.join()
        robot_server_thread.client.arm.set_learning_mode(True)
        for client_thread in robot_clients_threads:
            client_thread.client.arm.set_learning_mode(True)
        exit()
    ...
    class RobotLeader(Thread):
        ...
        def run(self):
            ...
            self.client.arm.calibrate(CalibrateMode.AUTO)
            self.client.arm.set_learning_mode(False)
            self.client.move_joints(*self.parent.desired_joint)
            self.client.arm.set_learning_mode(True)
    ...
    class RobotFollower(Thread):
        ...
        def run(self):
            ...
            self.client.connect(self.robot_ip)
            self.client.arm.calibrate(CalibrateMode.AUTO)
            self.client.arm.move_joints(*self.parent.desired_joint)
