...
###### VARIABLES TO ADAPT AND CHANGE #############
robot_ip_address = "10.10.10.10"  # Replace by robot ip address
workspace = "ww"  # Name of your workspace

# position for the robot to watch the workspace
observation_pose = (0.1882, -0.00146, 0.1995, 3.105, 1.375, 3.106)

# position for the robot to drop the object
drop_pose = (-0.0032, 0.3247, 0.1522, 3.128, 1.366, -1.568)
...
tool_id = client.tool.get_current_tool_id()
if tool_id == 11 or tool_id == 12 or tool_id == 13:  # If it is a gripper
    client.tool.close_gripper()  # close gripper so that workspace is more visible
client.arm.move_pose(observation_pose)
...
def get_all_objs():
    ...
    if not a:
        a, img_work = pyniryo.debug_markers(img_work)
...
def close_aplication(client):
    client.arm.move_joints(sleep_joints)
    client.arm.set_learning_mode(True)
    pygame.quit()
    client.end()
    exit(0)
...
def init_all():
    ...
    main_menu.add.button('Quit', close_aplication, client, font_size=font_size)
...
def pick_by_name(name):
    ...
    id_ = client.tool.get_current_tool_id()
    if id_ is None:
        return
    ...
    for x in range(len(objs)):
        ...
        if objects_names[objs[x].type] == name:
            print("object find")
            obj = client.vision.get_target_pose_from_rel(workspace, z_offset, objs[x].x / shape[0], objs[x].y / shape[1],
                                                  objs[x].angle)
            print("obj : ", obj)
            client.pick_place.pick_from_pose(*obj.to_list())
            client.pick_place.place_from_pose(drop_pose)
            client.tool.close_gripper()
            break
    client.arm.move_pose(observation_pose)
...
def change_observation_pose():
    while 1:
        ...
        while not wait_key_pressed(0):
            mtx, dist = client.vision.get_camera_intrinsics()
            img_compressed = client.vision.get_img_compressed()
            img_raw = pyniryo.uncompress_image(img_compressed)
            img = pyniryo.undistort_image(img_raw, mtx, dist)

            a, img2 = pyniryo.debug_markers(img)
            color = [255, 0, 0]
            if a:
                img = img2
                color = [0, 255, 0]
            ...
            surface.blit(surf, (window_x - window_y, 0))
            pygame.display.flip()
        pose = client.arm.get_pose()
        client.arm.set_learning_mode(False)
        client.arm.move_pose(pose.to_list())
        mtx, dist = client.vision.get_camera_intrinsics()
        img_compressed = client.vision.get_img_compressed()
        img_raw = pyniryo.uncompress_image(img_compressed)
        img = pyniryo.undistort_image(img_raw, mtx, dist)
